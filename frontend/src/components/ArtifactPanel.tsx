  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/5 p-5">
      <div className="mb-4">
        ...
        <p className="text-base text-slate-300">{manifest?.summary ?? 'No workspace yet.'}</p>
      </div>
      <div className="grid gap-4 lg:grid-cols-[18rem_minmax(0,1fr)]">
        <div className="space-y-2">
          {artifactEntries.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-white/10 px-4 py-6 text-base text-slate-400">
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-950/70 p-4">
          {isLoading && selectedArtifact ? (
            <div className="space-y-3">
              <div className="flex items-center gap-3 text-base text-cyan-300">
            ...
            <div className="text-base text-slate-400">Select an artifact to preview it.</div>
          ) : preview.kind === 'json' ? (
            <pre className="max-h-[26rem] overflow-auto whitespace-pre-wrap text-base text-slate-100">
