...
export function GoalInput({ goal, isRunning, onGoalChange, onSubmit }: GoalInputProps) {
  return (
    <section className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-2xl shadow-black/20 backdrop-blur">
      ...
      <textarea
        className="min-h-32 w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3.5 text-base text-slate-100 outline-none ring-0 placeholder:text-slate-500"
      ...
      />
      <div className="mt-2 text-sm text-slate-500">Press `Cmd/Ctrl + Enter` to run.</div>
    </section>
  )
}
