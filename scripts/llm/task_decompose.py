...
def decompose_task(task_text: str, *, model: str) -> TaskBreakdown:
    ...
    system = (
        ...
        "5. `build_features` — execute the feature/alpha plan to create engineered columns + target "
        "(auto-selects between feature_engineering and alpha_engineering skill based on workspace; "
        "`build_alphas` is an alias for the same tool)\n"
        "6. `train_model` — fit a regression model (ridge, lasso, RF, GBM, etc.) when the goal is predictive/ML-based\n"
        ...
        "`build_features` auto-selects alpha mode when workspace has `search_context` or `alpha_plan`. "
        "You can also use the alias `build_alphas` in subtask titles for clarity.\n"
